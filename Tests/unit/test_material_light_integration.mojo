...
def test_light_source_distant_direction_and_rgb() raises:
    ...
    assert_true(_close(s_ptr[unsafe_offset=0].distant_dirs[0], Float32(0.0)))
    assert_true(_close(s_ptr[unsafe_offset=0].distant_dirs[1], Float32(0.0)))
    assert_true(_close(s_ptr[unsafe_offset=0].distant_dirs[2], Float32(-1.0)))
    assert_true(_close(s_ptr[unsafe_offset=0].distant_rgbs[0], Float32(2.0)))
    assert_true(_close(s_ptr[unsafe_offset=0].distant_rgbs[1], Float32(3.0)))
    assert_true(_close(s_ptr[unsafe_offset=0].distant_rgbs[2], Float32(4.0)))
    scanner_free(handle)
...
def test_light_source_point_position_and_rgb() raises:
    ...
    assert_true(_close(s_ptr[unsafe_offset=0].point_pos[0], Float32(1.0)))
    assert_true(_close(s_ptr[unsafe_offset=0].point_pos[1], Float32(2.0)))
    assert_true(_close(s_ptr[unsafe_offset=0].point_pos[2], Float32(3.0)))
    assert_true(_close(s_ptr[unsafe_offset=0].point_rgbs[0], Float32(0.5)))
    assert_true(_close(s_ptr[unsafe_offset=0].point_rgbs[1], Float32(0.5)))
    assert_true(_close(s_ptr[unsafe_offset=0].point_rgbs[2], Float32(0.5)))
    scanner_free(handle)
    _ = s_ptr.take_pointee(); s_ptr.unsafe_free()
...
def test_light_source_infinite_no_filename() raises:
    ...
    assert_true(s_ptr[unsafe_offset=0].inf_tex_idx[0] == Int32(-1))
    assert_true(_close(s_ptr[unsafe_offset=0].inf_rgb[0], Float32(0.1)))
    assert_true(_close(s_ptr[unsafe_offset=0].inf_rgb[1], Float32(0.2)))
    assert_true(_close(s_ptr[unsafe_offset=0].inf_rgb[2], Float32(0.3)))
    scanner_free(handle)
    _ = s_ptr.take_pointee(); s_ptr.unsafe_free()
...
def test_light_source_infinite_with_filename() raises:
    ...
    assert_true(len(s_ptr[unsafe_offset=0].inf_tex_idx) == 1)
    assert_true(s_ptr[unsafe_offset=0].inf_tex_idx[0] == Int32(0))
    assert_true(len(s_ptr[unsafe_offset=0].tex_files) == 1)
    assert_true(s_ptr[unsafe_offset=0].tex_files[0] == String("textures/sky.exr"))
    assert_true(_close(s_ptr[unsafe_offset=0].inf_rgb[0], Float32(1.0)))
    scanner_free(handle)
    _ = s_ptr.take_pointee(); s_ptr.unsafe_free()
...
def test_area_light_source_rgb_l_scaled() raises:
    ...
    assert_true(_close(s_ptr[unsafe_offset=0].cur_attr.al_rgb.r, Float32(1.0)))
    assert_true(_close(s_ptr[unsafe_offset=0].cur_attr.al_rgb.g, Float32(2.0)))
    assert_true(_close(s_ptr[unsafe_offset=0].cur_attr.al_rgb.b, Float32(3.0)))
    scanner_free(handle)
    _ = s_ptr.take_pointee(); s_ptr.unsafe_free()
...
def test_named_material_conductor_rgb_eta_k_fresnel() raises:
    ...
    assert_true(s_ptr[unsafe_offset=0].named_materials[0].kind == MatKind.conductor)
    assert_true(_close(s_ptr[unsafe_offset=0].named_materials[0].albedo.r, Float32(0.923372)))
    assert_true(_close(s_ptr[unsafe_offset=0].named_materials[0].albedo.g, Float32(0.634888)))
    assert_true(_close(s_ptr[unsafe_offset=0].named_materials[0].albedo.b, Float32(0.476813)))
    scanner_free(handle)
    _ = s_ptr.take_pointee(); s_ptr.unsafe_free()
...
def test_named_material_dielectric_float_eta() raises:
    ...
    assert_true(len(s_ptr[unsafe_offset=0].named_materials) == 1)
    assert_true(s_ptr[unsafe_offset=0].named_materials[0].kind == MatKind.dielectric)
    assert_true(_close(s_ptr[unsafe_offset=0].named_materials[0].ior, Float32(1.33)))
    scanner_free(handle)
    _ = s_ptr.take_pointee(); s_ptr.unsafe_free()
...
def test_named_material_mix_names_and_amount() raises:
    ...
    assert_true(len(s_ptr[unsafe_offset=0].named_materials) == 1)
    assert_true(s_ptr[unsafe_offset=0].named_materials[0].kind == MatKind.mix)
    assert_true(s_ptr[unsafe_offset=0].named_materials[0].mix_name1 == String("matA"))
    assert_true(s_ptr[unsafe_offset=0].named_materials[0].mix_name2 == String("matB"))
    assert_true(_close(s_ptr[unsafe_offset=0].named_materials[0].mix_amount, Float32(0.25)))
    scanner_free(handle)
    _ = s_ptr.take_pointee(); s_ptr.unsafe_free()
