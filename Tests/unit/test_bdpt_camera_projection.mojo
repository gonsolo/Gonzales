...
def _make_camera() -> Tuple[
    ...
    r2c[unsafe_offset=0]  = Float32(2.0) / Float32(FW)        # fx -> x
    r2c[unsafe_offset=12] = Float32(-1.0)
    r2c[unsafe_offset=5]  = Float32(-1.5) / Float32(FH)       # fy -> y (film y is flipped)
    r2c[unsafe_offset=13] = Float32(0.75)
...
def test_world_to_raster_round_trips_the_camera_transform() raises:
    ...
    for ty in range(0, Int(FH), 5):
        for tx in range(0, Int(FW), 5):
            ...
            var cx = r2c[unsafe_offset=0]*fx0 + r2c[unsafe_offset=4]*fy0 + r2c[unsafe_offset=12]
            var cy = r2c[unsafe_offset=1]*fx0 + r2c[unsafe_offset=5]*fy0 + r2c[unsafe_offset=13]
            var cz = r2c[unsafe_offset=2]*fx0 + r2c[unsafe_offset=6]*fy0 + r2c[unsafe_offset=14]
