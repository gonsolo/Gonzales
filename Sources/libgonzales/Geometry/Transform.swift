...
extension Transform {
        // Returns the matrix as 16 Float32s in column-major order (col0..col3).
...
extension Transform {
        static public func makePerspective(
                fov: Angle, near: Distance, far _: Distance
        ) throws
                -> Transform
        {
                let persp = Matrix(
        ...
        static func makeScale(x: Real, y: Real, z: Real) throws -> Transform {
                let matrix = Matrix(
