...
from std.sys import has_accelerator, has_nvidia_gpu_accelerator, num_logical_cores
...
def mojo_render_all_tiles(
    ...
    var tw = Int(tile_w)
    var th = Int(tile_h)
    var n_tiles_x = (res_x + tw - 1) // tw
    var res_y = Int(max_y - min_y)
    var n_tiles_y = (res_y + th - 1) // th
    var n_tiles = n_tiles_x * n_tiles_y

    fn render_tile(tile_idx: Int) capturing:
        var tiy = tile_idx // n_tiles_x
        var tix = tile_idx % n_tiles_x
        var tx = ox + tix * tw
        var ty = oy + tiy * th
        var tx_max = min(tx + tw, ox + res_x)
        var ty_max = min(ty + th, oy + res_y)
        var tw_actual = tx_max - tx
        var th_actual = ty_max - ty
        var tile_buf = alloc[TileResult_C](tw_actual * th_actual)
        mojo_render_tile_v2(
            raster_to_camera, camera_to_world,
            Int32(tx), Int32(ty), Int32(tx_max), Int32(ty_max),
            sampler_params, scene, tile_buf, max_depth)
        for iy in range(th_actual):
            for ix in range(tw_actual):
                var src = iy * tw_actual + ix
                var dst = (ty - oy + iy) * res_x + (tx - ox + ix)
                results[dst] = tile_buf[src]
        tile_buf.free()

    parallelize[render_tile](n_tiles, num_logical_cores())
...
struct _PscState:
    # Transform stack (column-major)
    var ctm:       UnsafePointer[Float32, MutAnyOrigin]   # 16 floats, current CTM
    var ctm_stack: UnsafePointer[Float32, MutAnyOrigin]   # PSC_CTM_DEPTH * 16
    var ctm_depth: Int32

    # Attribute push/pop
    var attr_mat:    UnsafePointer[Int32, MutAnyOrigin]   # saved mat_name offsets (index into named)
    var attr_alight: UnsafePointer[Int32, MutAnyOrigin]   # saved in_alight
    var attr_al_rgb: UnsafePointer[Float32, MutAnyOrigin] # PSC_ATTR_DEPTH * 3
    var attr_depth:  Int32

    # Named materials: names (PSC_MAX_NAMED * PSC_NAME_MAX bytes) + albedo (PSC_MAX_NAMED * 3)
    ...
    # Film / filter / sampler / camera / integrator config
    var film_w: Int32
    var film_h: Int32
    var film_iso: Float32
    var film_max_comp: Float32
    var film_filename: UnsafePointer[UInt8, MutAnyOrigin]   # PSC_FILE_MAX bytes
    var filter_sigma: Float32
    var filter_support_x: Float32
    var filter_support_y: Float32
    var samples_per_pixel: Int32
    var camera_fov: Float32
    var cam2w_raw: UnsafePointer[Float32, MutAnyOrigin]  # 16 floats: CTM at Camera directive
    var max_depth: Int32
...
# Convert pbrt row-major [t00,t01,...,t33] to column-major flat[col*4+row].
...
fn _psc_type_is_str(t: UnsafePointer[UInt8, MutAnyOrigin]) -> Bool:
    ...
    # "string" starts with 'st'
    if t[0] == UInt8(115) and t[1] == UInt8(116): return True
    return False

# Skip a single parameter value (float, int, or string) from scanner.
# is_array=1 means '[' was already consumed; caller must scan ']' after.
fn _psc_skip_value(handle: UnsafePointer[PbrtScanner_Mojo, MutAnyOrigin],
    ...
    if _psc_type_is_float(type_buf):
        ...
        # bool or unknown: try to read a token
...
# Skip ALL remaining parameters of a directive.
...
# Skip rest of line (after '#' comment marker).
...
# --- Scan one float scalar param (for known-name parameters) ---
...
# Scan one string param (up to buf_max-1 bytes).
...
# Scan RGB triple (3 floats) into rgb[0..2].  is_array means '[' was consumed.
...
fn _psc_handle_integrator(handle: UnsafePointer[PbrtScanner_Mojo, MutAnyOrigin],
    ...
    _ = mojo_scanner_parse_quoted_string(handle, sbuf, 64)   # integrator name (ignored)
    var type_buf = alloc[UInt8](64)
    var name_buf = alloc[UInt8](128)
    var ia = alloc[Int32](1)
    ia[0] = Int32(0)
...
fn _psc_handle_sampler(handle: UnsafePointer[PbrtScanner_Mojo, MutAnyOrigin],
                       s: UnsafePointer[_PscState, MutAnyOrigin]):
    var sbuf = alloc[UInt8](64)
    _ = mojo_scanner_parse_quoted_string(handle, sbuf, 64)   # sampler name (ignored)
    var type_buf = alloc[UInt8](64)
    var name_buf = alloc[UInt8](128)
    var ia = alloc[Int32](1)
    ia[0] = Int32(0)
...
fn _psc_handle_filter(handle: UnsafePointer[PbrtScanner_Mojo, MutAnyOrigin],
                      s: UnsafePointer[_PscState, MutAnyOrigin]):
    var sbuf = alloc[UInt8](64)
    _ = mojo_scanner_parse_quoted_string(handle, sbuf, 64)   # filter name (ignored)
    var type_buf = alloc[UInt8](64)
    var name_buf = alloc[UInt8](128)
    var ia = alloc[Int32](1)
    ia[0] = Int32(0)
...
fn _psc_handle_film(handle: UnsafePointer[PbrtScanner_Mojo, MutAnyOrigin],
                    s: UnsafePointer[_PscState, MutAnyOrigin]):
    var sbuf = alloc[UInt8](64)
    _ = mojo_scanner_parse_quoted_string(handle, sbuf, 64)   # film type (ignored)
    var type_buf = alloc[UInt8](64)
    var name_buf = alloc[UInt8](128)
    var ia = alloc[Int32](1)
    ia[0] = Int32(0)
...
fn _psc_handle_camera(handle: UnsafePointer[PbrtScanner_Mojo, MutAnyOrigin],
                      s: UnsafePointer[_PscState, MutAnyOrigin]):
    var sbuf = alloc[UInt8](64)
    _ = mojo_scanner_parse_quoted_string(handle, sbuf, 64)   # camera type (perspective)
    # Capture camera-to-world = inverse of current CTM
    _psc_matcopy(s[0].cam2w_raw, s[0].ctm)  # store CTM; finalize inverts it
    var type_buf = alloc[UInt8](64)
    var name_buf = alloc[UInt8](128)
    var ia = alloc[Int32](1)
    ia[0] = Int32(0)
...
fn _psc_handle_transform(handle: UnsafePointer[PbrtScanner_Mojo, MutAnyOrigin],
                         s: UnsafePointer[_PscState, MutAnyOrigin]):
    # pbrt Transform values are already column-major (Swift: t01=values[4], etc.) — copy directly.
...
fn _psc_handle_attribute_end(s: UnsafePointer[_PscState, MutAnyOrigin]):
    ...
    if s[0].attr_depth > 0:
        ...
        s[0].in_alight = Int32(0)  # reset for next attribute block

fn _psc_handle_area_light_source(handle: UnsafePointer[PbrtScanner_Mojo, MutAnyOrigin],
                                 s: UnsafePointer[_PscState, MutAnyOrigin]):
    var sbuf = alloc[UInt8](64)
    _ = mojo_scanner_parse_quoted_string(handle, sbuf, 64)   # "diffuse"
...
fn _psc_handle_shape(handle: UnsafePointer[PbrtScanner_Mojo, MutAnyOrigin],
    ...
    # Build vertex-index array (Int64) and face-index array (all 3)
...
fn _psc_parse(handle: UnsafePointer[PbrtScanner_Mojo, MutAnyOrigin],
    ...
    while mojo_scanner_is_at_end(handle) == 0:
        ...
        if _psc_streq(kw_buf, "Integrator"):
            ...
            # Unknown directive: skip any quoted-string name + all parameters
...
# --- Build perspective matrix (column-major) from fov_deg and near ---
# pbrt uses fov as both the fov angle and the far-plane value (by convention).
fn _psc_make_perspective(fov_deg: Float32, near: Float32,
    ...
    var far = fov_deg   # pbrt uses fov value as far (matching Swift's makePerspective)
...
# Build screenToRaster matrix (column-major).
...
fn _psc_finalize(s: UnsafePointer[_PscState, MutAnyOrigin],
    ...
    # ---- Camera matrices ----
    # cam2w_raw = CTM at Camera time; camera_to_world = its inverse.
    var c2w = alloc[Float32](16)
    _ = mojo_matrix_invert(s[0].cam2w_raw, c2w)
    psc[0].camera_to_world = c2w

    # Perspective matrix and screenToRaster → rasterToCamera
    ...
    # ---- Materials ----
    # One Material_C per named material (type=diffuse=1), then one per area-light (type=2).
    ...
    # al_mat_offset: material index for first area-light material
    ...
    # ---- BVH construction ----
    # Collect total triangle count
    var total_tris = Int32(0)
    for i in range(n_meshes):
        total_tris += s[0].mesh_nt[i]

    var prim_bounds = alloc[Float32](Int(total_tris) * 6)
    var tri_mesh    = alloc[Int32](Int(total_tris))   # which mesh each triangle belongs to
    var tri_local   = alloc[Int32](Int(total_tris))   # triangle index within mesh
    ...
    var al_counter = Int32(0)

    # First pass: count area lights per mesh to assign al indices
...
# Getter: SceneDescriptor2_C built from a ParsedScene_Mojo.
# Caller must free the returned heap-allocated struct.
@export
...
@export
fn mojo_parsed_raster_to_camera(psc: UnsafePointer[ParsedScene_Mojo, MutAnyOrigin],
                                 out16: UnsafePointer[Float32, MutAnyOrigin]):
    for i in range(16):
        out16[i] = psc[0].raster_to_camera[i]

@export
fn mojo_parsed_camera_to_world(psc: UnsafePointer[ParsedScene_Mojo, MutAnyOrigin],
                                out16: UnsafePointer[Float32, MutAnyOrigin]):
    for i in range(16):
        out16[i] = psc[0].camera_to_world[i]

@export
fn mojo_parsed_film_w(psc: UnsafePointer[ParsedScene_Mojo, MutAnyOrigin]) -> Int32:
    return psc[0].film_w

@export
fn mojo_parsed_film_h(psc: UnsafePointer[ParsedScene_Mojo, MutAnyOrigin]) -> Int32:
    return psc[0].film_h

@export
fn mojo_parsed_film_iso(psc: UnsafePointer[ParsedScene_Mojo, MutAnyOrigin]) -> Float32:
    return psc[0].film_iso

@export
fn mojo_parsed_film_max_comp(psc: UnsafePointer[ParsedScene_Mojo, MutAnyOrigin]) -> Float32:
    return psc[0].film_max_comp

@export
fn mojo_parsed_film_filename(psc: UnsafePointer[ParsedScene_Mojo, MutAnyOrigin],
                              buf: UnsafePointer[UInt8, MutAnyOrigin], max_buf: Int32):
    _psc_strncpy(buf, psc[0].film_filename, max_buf)

@export
fn mojo_parsed_filter_sigma(psc: UnsafePointer[ParsedScene_Mojo, MutAnyOrigin]) -> Float32:
    return psc[0].filter_sigma

@export
fn mojo_parsed_filter_support_x(psc: UnsafePointer[ParsedScene_Mojo, MutAnyOrigin]) -> Float32:
    return psc[0].filter_support_x

@export
fn mojo_parsed_filter_support_y(psc: UnsafePointer[ParsedScene_Mojo, MutAnyOrigin]) -> Float32:
    return psc[0].filter_support_y

@export
fn mojo_parsed_filter_norm_x(psc: UnsafePointer[ParsedScene_Mojo, MutAnyOrigin]) -> Float32:
    return psc[0].filter_norm_x

@export
fn mojo_parsed_filter_norm_y(psc: UnsafePointer[ParsedScene_Mojo, MutAnyOrigin]) -> Float32:
    return psc[0].filter_norm_y

@export
fn mojo_parsed_filter_weight(psc: UnsafePointer[ParsedScene_Mojo, MutAnyOrigin]) -> Float32:
    return psc[0].filter_weight

@export
fn mojo_parsed_samples_per_pixel(psc: UnsafePointer[ParsedScene_Mojo, MutAnyOrigin]) -> Int32:
    return psc[0].samples_per_pixel

@export
fn mojo_parsed_log2_spp(psc: UnsafePointer[ParsedScene_Mojo, MutAnyOrigin]) -> Int32:
    return psc[0].log2_spp

@export
fn mojo_parsed_n_base4_digits(psc: UnsafePointer[ParsedScene_Mojo, MutAnyOrigin]) -> Int32:
    return psc[0].n_base4_digits

@export
fn mojo_parsed_max_depth(psc: UnsafePointer[ParsedScene_Mojo, MutAnyOrigin]) -> Int32:
    return psc[0].max_depth

@export
fn mojo_parsed_rng_seed(psc: UnsafePointer[ParsedScene_Mojo, MutAnyOrigin]) -> UInt64:
    return psc[0].rng_seed

# === mojo_parse_and_render: parse .pbrt + render + write EXR in one call ===
