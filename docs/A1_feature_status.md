...
| 5 | **ReSTIR beyond one-bounce direct+indirect lighting** | ReSTIR DI (bounce 0), ReSTIR GI (one-bounce reconnection), and SMS-ReSTIR (temporal-only glass-caustic reuse) are all done. Phase 7 of `docs/A2_restir_migration_plan.md` (volumetric ReSTIR) has its RIS candidate generation done and wired (GPU wavefront); temporal/spatial reservoir reuse is written+unit-tested but not wired into the render loop. Phase 8 (BDPT-ReSTIR) not started. SMS-ReSTIR's own spatial reuse also remains. Phase 9 (unifying ReSTIR's reservoir formalism with VCM's connect/merge into one joint structure) is explicitly marked **open research** in the plan doc itself, not a scoped deliverable |
