...
| 7 | **Vulkan RT curve candidate cap on extreme-density hair** | `intersect_batch.comp` records only the first `CURVE_DEFER_K` (16) AABB candidates a ray encounters, in BLAS/TLAS traversal order, with no fallback for the rest (unlike the CUDA path, which always resolves correctly regardless of K — see `traverse_bvh2_core_defer_curves`). On curly-hair-scale density this silently drops real, sometimes-closer curve hits, producing visible bald patches. A "keep the K nearest by ray-AABB distance" fix was implemented and verified correct, but reading per-candidate AABB bounds in the shader regressed render time by 2+ orders of magnitude and was reverted (2026-08-04) — the underlying tradeoff (bounded per-ray candidate memory vs. no narrow-phase geometry access in a ray-query shader) needs a fundamentally cheaper approach, not just a bigger K |
