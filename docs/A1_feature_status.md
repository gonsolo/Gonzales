...
| # | Feature | Why it matters |
|---|---|---|
| 4 | **Vulkan RT scene coverage** | Currently hard-falls-back to CUDA for ANY scene with curves, spheres, or instances (`pipeline.mojo`'s `curve_count`/`sphere_count`/`instance_count` guard) — excludes hair scenes, sphere-light scenes, and instanced scenes (e.g. barcelona-pavilion) from the Vulkan RT speedup entirely. Ordered plan in `project_vulkan_rt_backend.md` memory: instancing (natural TLAS/BLAS fit) → spheres (tessellate for traversal only) → curves (hardest; needs either a custom intersection shader or a broad/narrow two-phase reuse of the existing CUDA curve-candidate pattern) |
