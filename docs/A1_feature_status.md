...
| Vulkan RT — 2nd GPU intersection backend | `a9eb370`+ | `VK_KHR_ray_query`; primary rays + diffuse-branch shadow rays, scene-adaptive on/off by light-path occupancy. Full scene coverage (2026-08-03, plain `--gpu --vulkan-rt-shade` batch path): meshes, object instancing (multi-geometry-per-template BLAS + real per-instance transforms), spheres (reuses the CUDA path's own analytic `test_spheres` pass unchanged), and curves (procedural-AABB BLAS feeding the existing curve-candidate deferral pipeline, no tessellation) all supported now. 2026-08-04: the extreme-density `vkAllocateMemory` OOM (curly-hair/bunny-fur, 1M+ curve leaves) is FIXED — the curve BLAS now builds in 250K-leaf chunks instead of one monolithic build, capping peak transient VRAM instead of needing the full scratch+AS buffer at once. Fixing that surfaced a real, previously-unreachable bug (Vulkan's "no triangle hit" path never reset `tHit`, so every curve candidate was silently rejected — rendered solid black) — also fixed. Curve candidate collection was then redesigned from a fixed-per-ray cap (caused bald patches on dense hair) to a workgroup-scan-based shared pool (bald patches now gone at every tested density) with its Vulkan-interop buffer read directly by the CUDA-side resolve kernel (no redundant copy). The separate VCM Vulkan RT path (bdpt.mojo) still excludes all of instancing/spheres/curves, not yet extended. See gap below for the real, quantified, VRAM-bound energy deficit that remains on the single densest known scene. |
...
| 7 | **Vulkan RT curve candidate pool still VRAM-capped on extreme-density hair** | 2026-08-04: `intersect_batch.comp`'s old fixed-per-ray candidate cap (silently dropped real hits once a ray exceeded it, causing visible bald patches) is REPLACED by a "count then place into a shared pool" scheme — each 64-ray workgroup does an exclusive prefix sum + one atomic reservation into a flat global pool, then re-traces to place candidates at their exact offsets. Verified via 3 standalone probes (dense-cluster recovery beyond the old cap, workgroup-boundary correctness, safe clamping under deliberate pool overflow — the clamp fix closed a real `CUDA_ERROR_ILLEGAL_ADDRESS` the redesign first introduced) and confirmed visually: bald patches are GONE at every tested density. The Vulkan-interop pool buffer is also now read DIRECTLY by `resolve_curve_candidates_gpu` instead of being copied into a same-sized CUDA-side duplicate, roughly halving memory cost per unit of capacity. Remaining gap: total pool capacity is still bounded by whatever VRAM survives the mesh/BVH/curve-BLAS build, and on curly-hair specifically (the densest known scene) the largest capacity that fits (48x rays) still leaves the render ~1.8x too dark (down from ~5.2x before this fix) — a real, quantified, VRAM-bound energy deficit, not a logic bug. A "keep the K nearest by ray-AABB distance" alternative was tried earlier and reverted (regressed render time 2+ orders of magnitude from per-candidate AABB-buffer reads) — not worth retrying without a fundamentally cheaper per-candidate ranking signal. |
