...
def nvdb_load(path: Pointer[UInt8, MutUntrackedOrigin], n: Int32) -> Pointer[UInt8, MutUntrackedOrigin]:
    ...
    return external_call["nvdb_load", Pointer[UInt8, MutUntrackedOrigin]](path, n)

def nvdb_data(handle: Pointer[UInt8, MutUntrackedOrigin]) -> Pointer[UInt8, MutUntrackedOrigin]:
    """The flat grid blob. memcpy-able straight to the GPU."""
    return external_call["nvdb_data", Pointer[UInt8, MutUntrackedOrigin]](handle)

def nvdb_size(handle: Pointer[UInt8, MutUntrackedOrigin]) -> Int64:
    return external_call["nvdb_size", Int64](handle)

def nvdb_free(handle: Pointer[UInt8, MutUntrackedOrigin]):
    external_call["nvdb_free", NoneType](handle)

# ── Test oracle (C bridge, host only -- NOT for rendering) ───────────────

def nvdb_get_value_ref(handle: Pointer[UInt8, MutUntrackedOrigin], i: Int32, j: Int32, k: Int32) -> Float32:
...
def nvdb_active_count(handle: Pointer[UInt8, MutUntrackedOrigin]) -> Int64:
...
def nvdb_active_coord(handle: Pointer[UInt8, MutUntrackedOrigin], n: Int64,
                      out3: Pointer[Int32, MutUntrackedOrigin]):
...
def nvdb_load_named(path: Pointer[UInt8, MutUntrackedOrigin],
    ...
    return external_call["nvdb_load_named", Pointer[UInt8, MutUntrackedOrigin]](path, grid_name)

def nvdb_index_bbox(handle: Pointer[UInt8, MutUntrackedOrigin],
                    out_min3: Pointer[Int32, MutUntrackedOrigin],
                    out_max3: Pointer[Int32, MutUntrackedOrigin]):
...
def nvdb_value_range(handle: Pointer[UInt8, MutUntrackedOrigin],
                     out_min: Pointer[Float32, MutUntrackedOrigin],
                     out_max: Pointer[Float32, MutUntrackedOrigin]):
...
def nvdb_map_invmatf(handle: Pointer[UInt8, MutUntrackedOrigin],
                     out9: Pointer[Float32, MutUntrackedOrigin]):
    """World-to-index 3x3 (row-major), PNanoVDB's inverse Map matrix."""
    external_call["nvdb_map_invmatf", NoneType](handle, out9)

def nvdb_map_vecf(handle: Pointer[UInt8, MutUntrackedOrigin],
                  out3: Pointer[Float32, MutUntrackedOrigin]):
...
@always_inline
def _nvdb_u32(blob: Pointer[UInt8, MutUntrackedOrigin], off: Int) -> UInt32:
    return (blob.unsafe_offset(off)).unsafe_bitcast[UInt32]()[unsafe_offset=0]

@always_inline
def _nvdb_u64(blob: Pointer[UInt8, MutUntrackedOrigin], off: Int) -> UInt64:
    return (blob.unsafe_offset(off)).unsafe_bitcast[UInt64]()[unsafe_offset=0]

@always_inline
def _nvdb_i64(blob: Pointer[UInt8, MutUntrackedOrigin], off: Int) -> Int64:
    return (blob.unsafe_offset(off)).unsafe_bitcast[Int64]()[unsafe_offset=0]

@always_inline
def _nvdb_f32(blob: Pointer[UInt8, MutUntrackedOrigin], off: Int) -> Float32:
...
@always_inline
def _nvdb_mask_is_on(blob: Pointer[UInt8, MutUntrackedOrigin], mask_base: Int, bit: Int) -> Bool:
