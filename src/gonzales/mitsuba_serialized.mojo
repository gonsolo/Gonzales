...
def _mit_ser_u32(buf: Pointer[UInt8, MutUntrackedOrigin], pos: Int) -> Int:
    return Int((buf.unsafe_offset(pos)).unsafe_bitcast[UInt32]()[unsafe_offset=0])

def _mit_ser_u64(buf: Pointer[UInt8, MutUntrackedOrigin], pos: Int) -> Int:
    return Int((buf.unsafe_offset(pos)).unsafe_bitcast[UInt64]()[unsafe_offset=0])

def _mit_ser_f32(buf: Pointer[UInt8, MutUntrackedOrigin], pos: Int) -> Float32:
    return (buf.unsafe_offset(pos)).unsafe_bitcast[Float32]()[unsafe_offset=0]

def _mit_ser_f64(buf: Pointer[UInt8, MutUntrackedOrigin], pos: Int) -> Float64:
